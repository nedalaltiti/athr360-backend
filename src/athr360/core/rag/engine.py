...
class RAG:
    ...
    async def _retrieve_with_multi_strategy(self, query: str, k: int) -> List[RetrievedChunk]:
        ...
        search_tasks = [
            self._semantic_search(query, k * 2),  # Primary search with focused pool
            self._entity_aware_search(query, k),  # Entity-specific search
        ...
        return sorted_chunks[:k]  # Return focused chunks for concise responses
    ...
    def _format_chunks_for_prompt(chunks: List[RetrievedChunk]) -> str:
        ...
        # Prioritize high-confidence chunks but limit for concise responses
        # Reduced chunk count for shorter, more focused responses
        if high_conf:
            # For focused responses, limit high-confidence chunks
            selected_chunks = (high_conf[:4] + med_conf[:2] + low_conf[:1])[:6]
        else:
            # Standard selection if no high-confidence chunks
            selected_chunks = (high_conf[:3] + med_conf[:2] + low_conf[:1])[:5]
